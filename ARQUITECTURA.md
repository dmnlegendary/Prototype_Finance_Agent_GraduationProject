...
- Dentro de esos dos archivos, **una sola tabla** (la del estado "vista normal"/"carrito") se conectó de verdad al ORM con un `{% for %}` real (`productos` / `carrito_items`, con su `{% empty %}`), como ejemplo del patrón a seguir. Las demás tablas duplicadas (estados "alta", "modificar", "proveedores", "alertas" — que en el prototipo son copias fijas usadas solo para la demo de `showState()`) se dejaron con sus datos de ejemplo originales + un comentario `TODO` explicando que son vistas de demo pendientes de una arquitectura más dinámica.
- Cada archivo tiene comentarios `{# TODO: ... #}` en los puntos donde falta conectar datos reales o lógica de negocio.
...
4. Inventario: implementar las vistas de alta/edición de producto y proveedor (los formularios YA están maquetados en los paneles laterales de `panel.html`, solo faltan `views.py`).
5. Ventas: decidir dónde vive el carrito en curso (sesión vs. modelo `Venta` en estado `EN_CURSO`) e implementar agregar/quitar producto + cobrar.
...
7. Reemplazar las 4 "vistas de demo" duplicadas de `inventario.html`/`venta1.html` por una arquitectura dinámica real (una vista por acción + Fetch API), quitando las barras "Estado: 1·2·3..." que son solo ayudas de prototipo.
